//
// Ejemplo de suscripción al nodo de Bitcoin usando ZeroMQ - hashblock
//  
...
fn mempool_subscriber() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Err(e) = subscriber.set_subscribe(b"hashblock") {
        eprintln!("Error al suscribirse: {:?}", e);
        return Err(e.into());
    }

    println!("\n");
    println!("Esperando mensajes (hashblock) del nodo Bitcoin ...");
    println!("\n");
